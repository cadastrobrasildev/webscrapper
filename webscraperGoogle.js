  ...
  try {
    ...
    while (keepSearching) {
      ...
      const result1 = await pool1.query(`
        ...
        FROM industrias 
        WHERE  ${process.env.SCRAP_UF ? ` uf = '${process.env.SCRAP_UF}' and` : ''}
        email IS NULL OR tel1 IS NULL
        ORDER BY random()
        LIMIT 1
      `);
      ...
      if (isCaptchaPresent) {
        console.error(`[${new Date().toISOString()}] CAPTCHA DETECTED! Waiting 3 minutes before continuing.`);

        // Espera 3 minutos antes de tentar novamente
        const captchaWaitTime = 3 * 60 * 1000; // 3 minutos em milissegundos
        console.log(`[${new Date().toISOString()}] Cooling down for ${captchaWaitTime/1000} seconds...`);
        await sleep(captchaWaitTime);
        console.log(`[${new Date().toISOString()}] Resuming operations after cooldown`);
        continue; // Volta ao início do loop para tentar outro registro
      }
